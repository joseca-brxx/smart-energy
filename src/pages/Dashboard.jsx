...
export default function Dashboard() {
  ...
  const [medicionesPorEquipo, setMedicionesPorEquipo] = useState({})
  ...
  useEffect(() => {
    cargarMisEquipos()
    const intervalo = setInterval(cargarMisEquipos, 5000)
    return () => clearInterval(intervalo)
  }, [tieneAcceso, user?.id])

  const lecturasReales = misEquipos
    .filter((eq) => medicionesPorEquipo[eq.id])
    .map((eq) => ({
      device: { id: eq.id, nombre: eq.nombre },
      reading: {
        potenciaKw: medicionesPorEquipo[eq.id].potencia_w / 1000,
        estado: medicionesPorEquipo[eq.id].potencia_w > 20 ? 'encendido' : 'apagado',
      },
    }))

  // ---- Modo demostración (sin pago confirmado): catálogo por segmento ----
  const devicesDemo = useMemo(() => getDevices(segmento), [segmento])
  ...
  // Lo que se muestra en pantalla depende del modo
  const lecturas = tieneAcceso ? lecturasReales : lecturasDemo
  const consumoInstantaneoKw = lecturas.reduce((a, l) => a + l.reading.potenciaKw, 0)
  const mayorConsumo = lecturas.length > 0
    ? [...lecturas].sort((a, b) => b.reading.potenciaKw - a.reading.potenciaKw)[0]
    : null
  const porcentajeMayor = mayorConsumo && consumoInstantaneoKw > 0
    ...
    <div className="space-y-5">
      ...
      <section>
        <h2 className="text-sm font-semibold mb-2 flex items-center gap-1" style={{ color: 'var(--color-text)' }}>
          <Zap size={16} style={{ color: 'var(--color-primary)' }} /> Consumo en tiempo real {!tieneAcceso && '(demostración)'}
        </h2>

        {tieneAcceso && misEquipos.length === 0 && (
        ...
        <div className="space-y-2">
          {lecturas.map(({ device, reading }) => (
            <div
              ...
              style={{ background: 'var(--color-surface)', border: tieneAcceso ? '1px solid var(--color-primary)' : '1px solid var(--color-border)' }}
            >
              <div>
                ...
                <p className="text-[11px]" style={{ color: reading.estado === 'encendido' ? 'var(--color-primary)' : 'var(--color-text-dim)' }}>
                  {reading.estado === 'encendido' ? '● Encendido' : '○ Apagado'} · {tieneAcceso ? 'Dato real' : 'Simulado'}
                </p>
              </div>
      ...
      {lecturas.length > 0 && (
        <section className="rounded-2xl p-4" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
          ...
          {!tieneAcceso && alertasDemo.length > 0 && (
            <div className="space-y-2">
              {alertasDemo.map((a) => (
