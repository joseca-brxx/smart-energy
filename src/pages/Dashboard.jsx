...
export default function Dashboard() {
  ...
  // Los equipos de demostración (todo menos el real) reflejan el segmento
  // del plan (hogar/PyME). El historial de consumo, por ahora, también se
  // calcula solo con estos — cuando el ESP32 lleve varios días
  // transmitiendo, se puede sumar el mismo cálculo para el equipo real.
  const devicesDemo = useMemo(() => getDevices(segmento).filter((d) => d.source !== 'real'), [segmento])
  ...
  const lecturaReal =
    tieneAcceso && equipoReal && medicionReal
      ? {
    ...
    <div className="space-y-5">
      ...
      <section>
        ...
        <div className="space-y-2">
          {/* Tarjeta del equipo real — bloqueada sin pago confirmado */}
          {isSupabaseConfigured && equipoReal && !tieneAcceso && (
            <div className="rounded-xl p-3 flex items-center gap-2" style={{ background: 'var(--color-surface)', border: '1px dashed var(--color-border)' }}>
              ...
              <div>
                ...
                <p className="text-[11px]" style={{ color: 'var(--color-text-dim)' }}>Activa tu plan para ver este equipo con datos reales.</p>
              </div>
            </div>
          )}
          {isSupabaseConfigured && equipoReal && tieneAcceso && (
